...
// width/height must be on the element itself. An SVG with only a viewBox
// falls back to 300x150, which is why unsized icons blow up the layout.
// CSS rules that set an explicit size still override these.
const stroke = (d, extra = '') =>
  `<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">${d}${extra}</svg>`;
